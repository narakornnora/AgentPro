...
state.collections = state.collections || (SAMPLE.collections || {}); // bring sample named datasets if any
...
async function ensureApi(){
    try { const r = await fetch(`${API_BASE}/health`, {cache:'no-store'}); API_OK = r.ok; }
    catch(e){ API_OK = false; }
    return API_OK;
}
async function tryLoadCollectionsFromAPI(){
    const targets = new Set(
        [...Object.keys(state.collections||{}),
         ...((BP.routes||[]).flatMap(r => (r.components||[]).map(c => c.collection).filter(Boolean)))]
    );
    if(targets.size===0) return;
    if(!(await ensureApi())) return;
    for(const name of targets){
        const endpoint = name; // assume name is already plural, e.g., posts, users
        try{
            const resp = await fetch(`${API_BASE}/api/${endpoint}`, {cache:'no-store'});
            if(resp.ok){
                const data = await resp.json();
                const items = Array.isArray(data) ? data : (data.items||[]);
                if(Array.isArray(items)){ state.collections[name] = items; }
            }
        }catch(e){}
    }
    save();
    router(); // refresh current view with live data
}
...
window.addEventListener('load', ()=>{ router(); tryLoadCollectionsFromAPI(); });
